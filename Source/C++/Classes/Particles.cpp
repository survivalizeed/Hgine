...
void sur::Particles::Bind(bool Render)
{
    for (i32 i = 0; i < Offsets->size(); ++i)
    {
        ...
        if ((settings->middle - (Coords->at(i).pos + position + Offsets->at(i))).magnitude() > settings->max_distance_to_middle && settings->max_distance_to_middle != 0.f)
        {
        ...
        if (std::find(settings->block_directions.begin(), settings->block_directions.end(), Direction::Up) == settings->block_directions.end() && dir == 0)
            Offsets->at(i).y -= noise; //W
        if (std::find(settings->block_directions.begin(), settings->block_directions.end(), Direction::Left) == settings->block_directions.end() && dir == 1)
            Offsets->at(i).x -= noise; //A
        if (std::find(settings->block_directions.begin(), settings->block_directions.end(), Direction::Down) == settings->block_directions.end() && dir == 2)
            Offsets->at(i).y += noise; //S
        if (std::find(settings->block_directions.begin(), settings->block_directions.end(), Direction::Right) == settings->block_directions.end() && dir == 3)
            Offsets->at(i).x += noise; //D
    }
