	...
	for (;;) {
		renderer.ClearScreenBuffer();
		{
			...
			for (i16 i = 0; i < shotvar; ++i) {
				sur::Instancer::Get<sur::Object>("CloneShot" + std::to_string(i))->Bind(true, true, ColliderType::Filled);
				sur::Instancer::Get<sur::Object>("CloneShot" + std::to_string(i))->Move({ 0,-4 }, true);
			}
			for (i16 i = 0; i < shotenemyvar; ++i) {
				sur::Instancer::Get<sur::Object>("CloneShotE" + std::to_string(i))->Bind(true, true, ColliderType::Filled);
				sur::Instancer::Get<sur::Object>("CloneShotE" + std::to_string(i))->Move({ 0,8 }, true);
			}
		}
		{
