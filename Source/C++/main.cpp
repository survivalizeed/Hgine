...
int main() {
	...
	sur::Instancer::Add(new sur::Triangle({ 0,0 }, { 100,0 }, { 0,100 }, Color(125, 54, 125), "Tri", 123), Types::Triangle);
	...
	for (;;) {
		...
		{
			sur::Instancer::GetObj("Space")->Bind(true,false, ColliderType::None);
			sur::Instancer::GetObj("Enemy")->Bind(true, true, ColliderType::Filled);
			sur::Instancer::GetObj("Player")->Bind(true, true, ColliderType::Filled);
			sur::Instancer::GetLine("LineLeft")->Bind(false,true);
			sur::Instancer::GetLine("LineRight")->Bind(false,true);
			for (i16 i = 0; i < shotvar; ++i) {
				sur::Instancer::GetObj("CloneShot" + std::to_string(i))->Bind(true,true, ColliderType::Filled);
				sur::Instancer::GetObj("CloneShot" + std::to_string(i))->Move({ 0,4 }, true);
			}
			for (i16 i = 0; i < shotenemyvar; ++i) {
				sur::Instancer::GetObj("CloneShotE" + std::to_string(i))->Bind(true,true, ColliderType::Filled);
