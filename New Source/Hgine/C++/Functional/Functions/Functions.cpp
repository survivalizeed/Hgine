...
ACCESS_PRIVATE_FIELD(sur::Object, sur::i32, layer);

void sur::SetLayer(Object* obj, i32 layer)
{
    auto& var = access_private::layer(*obj);
    var = layer;
    _layer_changed = true;
}
...
sur::Vec2f sur::GetGeometryMetricsStart(Object* object)
{
    return { 0.f,0.f };
}

sur::Vec2f sur::GetGeometryMetricsEnd(Object* object)
{
    if (object->GetType() == Type::Line)
        return static_cast<Line*>(object)->end_point;
    if (object->GetType() == Type::Square)
        return static_cast<Square*>(object)->end_point;
    if (object->GetType() == Type::Sprite)
        return static_cast<Sprite*>(object)->size;
    if (object->GetType() == Type::Text)
        return static_cast<Text*>(object)->GetSize();
#ifdef _DEBUG
    Error("GetGeometryMetricsEnd() only works with Lines, Squares, Sprites and Texts");
#endif
    return { 0.f, 0.f };
}
...
std::string sur::GetClipBoardContent()
{
    HANDLE clipHandle;
    std::string ret;
    if (OpenClipboard(NULL)) {
        clipHandle = GetClipboardData(CF_TEXT);
        char* clipContent = static_cast<char*>(clipHandle);
        int counter = 0;
        while (clipContent[counter] != '\0') {
            ret += clipContent[counter];
            counter++;
        }
        CloseClipboard();
        return ret;
...
std::vector<std::string> sur::GetDirectoryFiles(std::string_view path)
{
    std::vector<std::string> entrys;
    for (const auto& entry : fs::directory_iterator(path))
        entrys.push_back(fs::absolute(entry).string());
    return entrys;
}
