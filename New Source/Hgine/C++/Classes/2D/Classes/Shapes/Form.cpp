	...
	if (render) {
		if (modifier == Modifier::None || modifier == Modifier::InverseKinematic) {
			if (renderMode == RenderMode::Wireframe)
				algorithm::DrawFormWire(points_with_position, TintPixel(color));
			else
				if (fillMode == FillMode::Auto)
					algorithm::DrawFormAuto(points_with_position, TintPixel(color));
				else if (fillMode == FillMode::Index)
					algorithm::DrawFormIndex(points_with_position, indices, TintPixel(color));
		}
	}
}