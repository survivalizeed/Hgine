...
sur::Font::Font(std::string_view file, i32 charCount, std::string_view align, Color colorToAlpha)
{
    ...
    for (i32 i = 0; i < charCount; ++i)
        chars.push_back(Sprite("Empty", Sprite::FileType::Empty, {0,0}, "EmptyCharSprite" + std::to_string(i), Color(0, 0, 0)));
    i32 offsetX = 0;
    for (i32 i = 0; i < charCount; ++i) {
        for (;;) {
            if (offsetX >= width)
    ...
    maxYOfSprites = Pixel(std::max_element(chars.begin(), chars.end(), [](Sprite& l, Sprite& r) {
        return l.size.y < r.size.y;
    })->size.y, Axis::Y);
}
