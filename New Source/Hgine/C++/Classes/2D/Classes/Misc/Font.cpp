...
sur::Font::Font(std::string_view file, i32 charCount, std::string_view align, Color colorToAlpha, std::string_view name)
{
    this->align = align;
    this->type = Type::Font;
    this->name = name;
    ...
    for (i32 i = 0; i < charCount; ++i)
        chars.push_back(Sprite("Empty", Sprite::FileType::Empty, {0,0}, "Empty", Color(0, 0, 0)));
    i32 offsetX = 0;
    for (i32 i = 0; i < charCount; ++i) {
        for (;;) {
            if (offsetX >= width)
...
std::string_view sur::Font::GetName() const
{
    return name;
}

sur::Type sur::Font::GetType() const
{
    return type;
}

sur::i32 sur::Font::GetHash() const
{
    return hash;
}
